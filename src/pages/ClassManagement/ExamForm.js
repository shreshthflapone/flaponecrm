...
const ExamForm = () => {
  ...
  const navigate  = useNavigate();
    useTitle("Exam Detail - Flapone Aviation");
  ...
  const openExamList = () => {
     navigate("/class-management/exam");
  };
  ...
  const handleDateChange = (date) => {
    setExamFormData((prevValues) => ({
      ...prevValues,
      examDate: moment(date).format("DD/MM/YYYY"),
    }));
  };
  ...
  const getAllDataForm = async () => {
    axios({
      ...
      if (responseData.status === "1") {
        ...
        setCourseListOptions(responseData.courseData);
        setBatchListOptions(responseData.subjectData);
        setSubjectListOptions(responseData.batchData);
      }
  ...
  return (
    <>
      {(filterStatus !== null && pageLoadStatus !== null) &&
        ...
        <InnerHeader
          heading="Add Exams"
          txtSubHeading="Use this form to add a exams. Provide the exam details and any other relevant information."
          showButton={true}
