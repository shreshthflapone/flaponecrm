...
const ViewStudentLogs = () => {
  ...
  const [selectedTab, setSelectedTab] = useState("student_log");
  ...
  const initial_obj = {
    ...
    log_id: StudentId != "all" ? StudentId : "",
    date: "",
    selectedBranch:[],
    selectedCourses:[],
    selectedCategory:[],
  ...
  const getStudentLogListRecord = async () => {
    setAutoLoader(true);
    setDisplayMsg("");
    axios({
      method: "post",
  ...
  useEffect(() => {
    if (StudentId !== "all") {
      const fetchStudentData = async () => {
        axios({
          method: "post",
          url: `${constant.base_url}/admin/inventory_list.php?fun=studentInfo`,
          headers: { "Auth-Id": user.auth_id },
          data: { 'log_id': StudentId},
        }).then(function (response) {
  ...
  // log pdf code
  const handleStudentLogsPDF = (studentid)=>{
     if(studentid == 'all'){
      handleShowMonthAccordingData();
    } else {
      handleShowMonthAccordingData(studentid);
    }
  }
  ...
  const handleShowMonthAccordingData = (studentid = null) => {
    axios({
      method: "post",
      url: `${constant.base_url}/admin/student_log_pdf_gen.php?fun=showLogMonthAccordingData`,
      headers: { "Auth-Id": user.auth_id },
      data: {
        type: "student_log",
        filter: listFilter,
  ...
  const handleDownloadClick = () => {
     ...
     const handleDownloadStudentPDF = (selectedmonth = null,selectedMonthStatus) => {
      setUpdateBtnStatus(true);
        axios({
          method: "post",
          url: `${constant.base_url}/admin/student_log_pdf_gen.php?fun=downloadstudentpdf`,
          headers: { "Auth-Id": user.auth_id },
          data: {
            filter: listFilter,
      ...
      {(filterApiStatus) && (
        <>
          <InnerHeader
            ...
            txtSubHeading={StudentId === "all" ? "View and manage flying logs across all students. Use filters to find specific records and monitor flying activity, hours logged, and performance trends." : `Review ${studentName}'s complete flying history, including hours logged, drone and battery usage, and instructor details. Add or edit logs as needed to maintain accurate records.`}
            showButton={true}
            iconText="Drone List"
              ...
              <Card className="bg5 mt16 pb16">
                ...
                Total Records: {totalCount} | Total Flying Hours: {totalFlyingCount}
                  <div className="v-center" style={{ gap: "12px" }}>
                   ...
                   {StudentId !== "all" && totalCount>0 &&  (
                    <button
                      onClick={() => {
                        if (!updateBtnStatus) {
                          handleStudentLogsPDF(StudentId);
                        }
                      }}
                ...
                >
                  <table className="mylead-table cp wsnw">
                    <thead className="w100">
                      <tr>
                        ...
                        <th style={{padding: '5px'}} rowSpan={2}>
                          <DynamicTooltip direction="left" text="Name of Remote Pilot">RPA Trainee</DynamicTooltip>
                        </th>
                        <th style={{padding: '5px'}} rowSpan={2}>
                    ...
                    {recordList.length > 0 ? (
                      ...
                      <tbody>
                        <tr>
                          <td colSpan={24} className="no-students">
                            No Log Found.
                          </td>
                        </tr>
                      </tbody>
          ...
          {addLogPopup && (
            ...
            >
              <AddLogForm
                ...
                multistudent={editMode?.id!==undefined ? false:true}
              />
            </Popup>
          )}
              ...
              <div className="log-popup">
                <Popup title="" onClose={cancelDroneSelectPopup}>
                  ...
                  <div className="popup-buttons mb24 df jcc">
                    ...
                    <button onClick={()=>{if (!updateBtnStatus) {handleDownloadClick()}}}  className={`update-button box-center mr24  ${updateBtnStatus ? 'disabled-input' : 'btn-blue'}`}>
                      Download PDF
                    </button>
                  </div>
                </Popup>
