...
const LocationList = () => {
  ...
  const limit = 10;
  useTitle("Location List - Flapone Aviation");
  ...
  const updateLocationPosition = (index,value,id) => {
    /*	  
    ...
    */
    ...
    updateLocationPostionApi(id,value);
  }
  ...
  const updateLocationPostionApi = async (id,value) => {
    axios({
      method: 'post',
      url: `${constant.base_url}/admin/flapone_list.php?fun=positionupdateloc`,
      headers: { "Auth-Id": user.auth_id },
      data: {"location_id": id,"value":value}
 ...
 const filterLabels = {
  ...
  const handleLogView = (id) => {
    if (id) {
      navigate("/location-log/" + id);
    } else {
      toast.error("No log available for this location");
    }
  };
  ...
  return (
    <>
     ...
     {(pageRoleAccess || pageContentAccessDept) && (<>
      <InnerHeader
        heading="Branches"
        txtSubHeading="View and manage all branches here. Use the options provided to add new branches or edit existing ones."
        showButton={true}
        iconText="Add New Branch"
        onClick={openCategoryAddPage}
      />
          ...
          <table className="wsnw list-blog">
            <thead className="w100">
              ...
              <th onClick={() => handleSortByChange("id")}>
                ...
                <th onClick={() => handleSortByChange('location_type')}  style={{ minWidth: "200px" }}>
                  <p className="box-center">
                    Location Type
                    <RiArrowUpDownFill className="cp ml4" />
                  </p>
                ...
                <th onClick={() => handleSortByChange("locality")} style={{ minWidth: "120px" }}>
                  <p className="box-center">
                  Locality
                    <RiArrowUpDownFill className="cp ml4" />
                  </p>
            ...
            <tbody>
              {sortList.map((location,index) => (
                <tr key={location.id}>
                  <td>{location.id}</td>
                  <td
                    style={{
                      color: giveTextColor(
                        location.location_type === "Head Office"
                          ? "#e38e03"
                          : location.location_type === "branch"
                          ? "#08d567"
                          : location.location_type === "center"
                          ? "#0039f2"
                  ...
                  <td>
                      <p className="text-row lc2 lh18">{location.title}</p>
                  </td>
                  <td>
                    {location.locality}
                  </td>
                  ...
                  <td>{location.create_date}</td>
                  <td>{location.update_date}</td>
                  <td>
		    {user.role === '1'?
                    <input
                      type="number"
                      ...
                      style={{ 
                        backgroundColor: location.location_status!=="1" ? '#f9f9f9' : '',
                        cursor: location.location_status!=="1" ? 'not-allowed' : '' 
                      }}
                      onInput={(e) => {
                      ...
                      onChange={(e) => {
                        ...
                        if (value.length <= 2) {
                          updateLocationPosition(index,value,location.id); 
                        }
                      }}
                      onFocus={(e) => {
                      ...
                      autoComplete="off"
                      readOnly={location.location_status!=="1"}
                  ...
                  <td  className="box-center">
                    <div className="action-icons box-center">
                      {location.checklog === 1 && (
                        <Tooltip title="View Logs">
                          <FaEye
                            ...
                            onClick={() => handleLogView(location.id)}
                      ...
                      <Tooltip title="Edit">
                        <FaPencilAlt
                          title="Edit"
                          onClick={()=>openDetailPage(index,location.id)}
