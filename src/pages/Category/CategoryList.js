...
const CategoryList = () => {
  useTitle("Category List - Flapone Aviation");
...
/*  const updateCategoryPosition = (index,value,id) => {
    recordList[index]['position'] = value?value:"";
    setRecordList([...recordList]);
    updateCategoryPostionApi(id,recordList[index]['position']);
  }
  */
...
const updateCategoryPosition = (index,value,id) => {
    ...
    */
    ...
    updateCategoryPostionApi(id,value);
  }


  const updateCategoryPostionApi = async (id,value) => {
    axios({
      method: 'post',
      url: `${constant.base_url}/admin/flapone_list.php?fun=positionupdatecategory`,
  ...
  return (
    <>
     {(pageRoleAccess || pageContentAccessDept) && ( <>
      <InnerHeader
        heading="Categories"
        txtSubHeading="View and manage all your categories here. Use the options provided to add new categories or edit existing ones."
        showButton={true}
        iconText="Add New Category"
              ...
              {sortList.map((blog,index) => (
                <tr key={blog.id}>
                  ...
                  <td
                    style={{
                      color: giveTextColor(
                        blog.article_type === "Home"
                          ? "home"
                          : blog.article_type === "Drone Training"
                            ? "dtrain"
                            : blog.article_type === "Aircraft Training"
                              ? "atrain"
                              : blog.article_type === "aircrafts"
                                ? "News"
                                : blog.article_type
                      ),
		    ...
		    {user.role === '1'?
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (value.length <= 2) {
                          updateCategoryPosition(index,value,blog.id); 
                        }
                      }}
                      onInput={(e) => {
