...
const Hot = ({
  ...
  actionAccess
          ...
          useTitle("Hot - Flapone Aviation");
  
  const [hotLeadData, setHotLeadData] = useState(recordList);
  const [assigned, setAssigned] = useState({});
  const [selectedLeads, setSelectedLeads] = useState([]);
  const [isAssignToPopupVisible, setAssignToPopupVisible] = useState(false);
  const navigate = useNavigate();
  useEffect(() => {
    setHotLeadData([...recordList]);
  }, [recordList]);
  ...
  const handleAssignToClick = () => {
    if (assigned.value) {
      const updatedLeads = hotLeadData.map((lead) => {
  ...
  const handleHeaderCheckboxChange = (event) => {
    const allLeadIds = hotLeadData.map((lead) => lead.userid);
    setSelectedLeads(event.target.checked ? allLeadIds : []);
  };

  const isHeaderCheckboxChecked =
    selectedLeads.length === hotLeadData.length && selectedLeads.length > 0;
