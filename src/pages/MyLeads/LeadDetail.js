...
import StudentRegistrationForm from "../../components/Forms/StudentRegistrationForm.js";
import StudentDownloadCertificate from "../../components/Forms/StudentDownloadCertificate.js";
...
const LeadDetail = () => {
  ...
  const RegistrationButtonDeptLimit = ["3","5","9"];
  const RegistrationButtonPermission = RegistrationButtonDeptLimit.includes(userDept);


  const paymentDeptLimit = ["3","4","5","7","9"];
  const paymentPermission = paymentDeptLimit.includes(userDept);

  

  const editPermission = (pageRoleAccess === true || pageLeadDetailAccessDept === true )? true:false;
 
  const [salesHierarchyAccess,setSalesHierarchyAccess] = useState(false);
  ...
  const [filterHistoryOptions, setFilterHistoryOptions] = useState([]);
  ...
  const [selectedOptionf, setSelectedOptions] = useState([]);
 
  //Shreshth Registration Form PDF Download
  const [enrolledCoursesStatus, setEnrolledCoursesStatus] = useState(false);
 
  //Shreshth Certificate Form PDF Download 
  ...
  const handleCallNow = async(type,user_data)=>{
      if(type=='callnow'){
        setCallNow(true);
      }
      axios({
        method: "post",
        url: `${constant.base_url}/admin/mylead_detail.php?fun=callnow`,
    ...
    axios({
      ...
      .then(function (response) {
        ...
        if (response.data.data.status === "1") {
          ...
          setSalesHierarchyAccess(otherData.saleshierarchyaccess);
          ...
          setEnrolledCoursesStatus(otherData.enrolledCoursesStatus);
          setCertificateStatus(otherData.certificateStatus);
        } else {
          navigate("/");
        }
      })
    ...
    if (user_id) {
      axios({
        ...
        .then(function (response) {
          ...
          const historyfilter = response.data.data.historyfilter;
          setCommentData(stsData);
          setFilterHistoryOptions([...historyfilter])
        })
        .catch(function (error) {
          console.error("Error during login:", error);
        });
  ...
  const handleSaveUpdateData = (updatedata,type='') => {
    setErrorMsg([]);
    // return false;
    axios({
      method: "post",
      url: type=='batch_allotment'?`${constant.base_url}/admin/mylead_detail.php?fun=updateBatch`:`${constant.base_url}/admin/mylead_detail.php?fun=updateLeadStatus`,
      headers: { "Auth-Id": user.auth_id },
      data: {
        user_id: userDetail.user_id,
      ...
      if(type=='batch_allotment' && respdata.status === "0" && respdata.msg=='batch not select'){
        toast.error("Please select batch");
        setUpdateStausPopup(false);
      }
      else if(type=='batch_allotment' && respdata.status === "0"){
        setErrorMsg(respdata.msg);
        setUpdateBtnStatus(false);
      }
  ...
  const handleRedirectStudentFromLeadDetail = (user_id) => {
    const updateStudentFilter = {
      ...
      "dateRangeValue": "27-08-2025 | 27-08-2025",
      "dateRangeValuefilter": [
        {
          "startDate": "2025-08-27T05:03:07.861Z",
          "endDate": "2025-08-27T05:03:07.861Z",
          "key": "selection"
        }
      ],
      "dateTimeType": "Today",
    ...
    oldFilter[tabParam] = updateStudentFilter;
    localStorage.setItem("allfilterstudent", JSON.stringify(oldFilter));

    let urlred = "https://my.flapone.com/my-students/all";
    //let urlred="http://localhost:3000/my-students/all";
    window.open(urlred);
  ...
  const handleClickedLog = async (user_id)=>{
    window.open("/student-logs/"+user_id);
  }
  ...
  //  const getUserDetailForLog = async ($log_user_id) => {
  //   axios({
  //     method: "post",
  //     url: `${constant.base_url}/admin/student_drone_log.php?fun=getuserlogdata`,
  //     headers: { "Auth-Id": user.auth_id },
  //     data: {
  //       user_id: id,
  //     },
  ...
  // };
  const handleAddLogs = () => {
  //getUserDetailForLog(userDetail.user_id);
  ...
  const handleSelectCommentFilter = (selectdoptionval) =>{
    setSelectedOptions(selectdoptionval);
  }

  //================== Registration Form ==================
  const [checkingDownloadRegFormSts,setCheckingDownloadRegFormSts] = useState(false);
  const checkingDownloadRegForm = async () => {
        setCheckingDownloadRegFormSts(true);
  }

  

  //================== Certificate PDF Download ==================
  const [checkingDownloadCertificateSts,setCheckingDownloadCertificateSts]=useState(false);

  function checkingDownloadCertificate (){
     setCheckingDownloadCertificateSts(true);
  }
        ...
        <div className="left-side-detail">
          <div className="detail-header mb16 df jcsb fww pl12 desk-lead-detail">
            ...
            <div className="header-right df fww ais">
              ...
              {
                userDetail.activeflyingbatch > 0 &&
                (user.role === '1' || user.dept_id === "3" || user.dept_id === "9") && (
                  <button
  ...
  <div className="sale-status box-center fdc cp" onClick={handleHistoryClick}>
    ...
    <p className="fc13">
      ...
      <span className="fc16 fs12">
        <DynamicTooltip text="Last comment(LSD)/follow-up(F) date">
          {userDetail.last_wo_created_date}
            ...
            <div className="sale-status box-center fdc cp" onClick={()=>handleClickNavigte(userDetail.redirect)}>
              ...
              <p className={`${userDetail.payment_workorder_status === 'Success' ? "fc13" : "fc6"}`}>{userDetail.payment_workorder_status === 'Success' ? "Completed" : userDetail.payment_workorder_status} 
  </p></div>}
              ...
              course.redirect ? (
                <div key={index} className="corner-box sale-status box-center fdc company-detail-info pr">
                  ...
                  <p className="brd-b1 pb4 box-center">
                    <span className="cp" onClick={()=>handleClickNavigte(course.redirect_course)}> <DynamicTooltip
                      direction="bottom"
                      text={`Click to view course details`}
                      onClickFunction={() => handleClickNavigte(course.redirect_course)}
                    >{course.course_name}</DynamicTooltip>{" "}</span>
                    <DynamicTooltip
                      direction="bottom"
                  ...
                  <p className="fc13">
                    <span  className="cp" onClick={()=>handleClickNavigte(course.redirect_course)} style={{color: giveTextColor(statusColorMap[course.course_status] || course.course_status)}}><DynamicTooltip
                    direction="bottom"
                      text={`Click to view course details`}
                      onClickFunction={() => handleClickNavigte(course.redirect_course)}
                    >{course.course_status}</DynamicTooltip>{" "}</span>
                     {(course.course_status!='Course Completed' && course.course_status!='Batch Completed') &&(<span className="fc16 fs12 cp" onClick={()=>handleClickNavigte(course.redirect_wid)}><DynamicTooltip
                    direction="bottom"
                      text={`Click to view payment details`}
                      onClickFunction={() => handleClickNavigte(course.redirect_wid)}
                    >{course.course_wo_create_date}</DynamicTooltip></span>)}
                    {(course.course_status=='Course Completed' || course.course_status=='Batch Completed') &&(<span className="fc16 fs12 cp" onClick={()=>handleClickNavigte(course.redirect_course)}><DynamicTooltip
                    direction="bottom"
                      text={`Batch/Course completed date`}
                      onClickFunction={() => handleClickNavigte(course.redirect_course)}
                    >{course.course_wo_create_date}</DynamicTooltip></span>)}
                  </p>
                </div>
                ...
                <div key={index} className="sale-status box-center fdc">
                  <p className="brd-b1 pb4 cp" onClick={()=>handleClickNavigte(course.redirect_course)}>
                    <DynamicTooltip
                      direction="bottom"
                      text={`Click to view course details`}
                      onClickFunction={() => handleClickNavigte(course.redirect_course)}
                    >{course.course_name}</DynamicTooltip></p>
                  <p className="fc25">
                    <span  className="cp" onClick={()=>handleClickNavigte(course.redirect_course)}  style={{color: giveTextColor(statusColorMap[course.course_status] || course.course_status)}}>  <DynamicTooltip
                    direction="bottom"
                      text={`Click to view course details`}
         ...
         {userDetail?.course_company_inv?.map((course, index) => (
          ...
          <p className="brd-b1 pb4 cp" onClick={()=>handleClickNavigte(course.redirect_course)}>{course.course_name}</p>
           <p className="">
           <span  className="cp" onClick={()=>handleClickNavigte(course.redirect_course)}><DynamicTooltip text={`Total allowed students | Click to view enrolled student list`} direction="right">T:{course?.T}</DynamicTooltip> | <DynamicTooltip  text={`Pending students for enrollment`} direction="right">P:{course?.P}</DynamicTooltip></span> <span className="fc16 fs12 cp" onClick={()=>handleClickNavigte(course.redirect_wid)}><DynamicTooltip text={`WOD: Workorder Created Date | Click to view payment details`}>{course.course_wo_create_date}</DynamicTooltip></span>
           </p>
         </div>
         ))}
            {userDetail?.student_attendance?.map((student, index) => (
              ...
              <p className="">
                <DynamicTooltip text={`T = Total Classes till today`}>T:{student.TC}</DynamicTooltip> | <DynamicTooltip text={`P = Present`}>P:{student.P} </DynamicTooltip> | <DynamicTooltip text={`A = Absent`}>A:{student.A}</DynamicTooltip> 
		{/*    <span className="fc16 fs12"> 
             ...
             {userDetail?.totalsecond >0 && <div className="sale-status box-center fdc cp" onClick={()=>handleClickedLog(userDetail.user_id)} >
              <p className="brd-b1 pb4">Total Flying</p>
              <p className="">
                <DynamicTooltip text={`Total flying hours till today`}>{userDetail?.total_flying_minute} Hours</DynamicTooltip>
          ...
          <Card className={"bg5"}>
            <div className="lead-detail-form-first form-row">
              <div className="form-group mr4">
                <SingleDropdown
                  ...
                  //isReadOnly={userDetail.user_account_type !== "Free" || workorderStatus}
                  //isReadOnly={otherDetail.paymentcount>0}
                  isReadOnly={otherDetail.paymentcount>0 || !(editPermission || salesHierarchyAccess)}
                />
              </div>
                ...
                <>
                  <div className="form-group mr4">
                    ...
                    <input
                      ...
                      disabled={!(editPermission || salesHierarchyAccess)}
                      className={editPermission  === false ? "bg6" : ""}
                  ...
                  <div className="form-group mr4">
                    ...
                    <input
                      ...
                      readOnly={workorderStatus || !(editPermission || salesHierarchyAccess)}
              ...
              <div className="form-row">
                <div className="form-group">
                  <SingleDropdown
                    ...
                    isReadOnly={workorderStatus || !(editPermission || salesHierarchyAccess)}
                  />
                </div>
                  ...
                  {!category ? (
                    <Tooltip title={"Please select category first"}>
                      <SingleDropdown
                        ...
                        isReadOnly={!category || workorderStatus || !(editPermission || salesHierarchyAccess)}
                ...
                <div className="form-group-segment course-full mb8 form-group-third">
                  <label
                    className={`${!category || workorderStatus || !(editPermission || salesHierarchyAccess) ? "mb24 " : "mb12"}`}
                  >
                    ...
                    <Tooltip title={"Please select category first"}>
                      <div className="form-group-segment course-full mt8 mb8 form-group-third">
                        <Select
                          ...
                          isDisabled={!category || workorderStatus || !(editPermission || salesHierarchyAccess)}
            ...
            <div className="df aic leadUpdate jcsb" ref={commentHistoryRef}>
              <p className="fs14">Comment History</p>
               <SingleDropdown
                options={filterHistoryOptions}
                selectedOption={selectedOptionf}
                onSelect={handleSelectCommentFilter}
              />
            </div>
            ...
            <div className="">
              ...
              <DynamicTooltip text="Click for View Student Profile" styleCustom={{ display: "block" }}>
                ...
                >
                  ...
                  <div>
                    <div className="student-link-detail box-center br50 fs18 fc3 bg1">
                ...
                <div className="form-row detail-side-bar lead-page-details">
                  <div className="ml8 status" style={{"width":"17%"}}>
                    <SingleDropdown
                      ...
                      isReadOnly={!(editPermission || salesHierarchyAccess)}
                      cc={true}
                    />
                  </div>
                  ...
                  {otherDetail.assignaccess ? (
                    <div className=" ml8 assigned-to rm"  style={{"width":"35%"}}>
                      <SingleDropdown
                        ...
                        isReadOnly={user.role === "1" || (user.dept_id === "4" || user.dept_id === "7")?false:true}
                        cc={true}
                      />
                    </div>
                  ) : (
                    <div className=" ml8 assigned-to rm tac"  style={{"width":"35%"}}>
                      <label className="brd-b1 w100  fc15 fw6 fs14 mb12 ls1">RM</label>
                      <input
                        className="pt8 ttc fs14 ls1 w100 tac"
                        type="text"
                        readOnly={true}
                  ...
                  {userDetail.stage === "paid" && (
                    <div className="mr8 rm"  style={{"width":"35%"}}>
                      <SingleDropdown
                        ...
                        cc={true}
                        isReadOnly={user.role === "1" || (user.dept_id === "3" || user.dept_id === "9")?false:true}
                      />
                    </div>
                  )}
                </div>
                <div className="lead-profile-info mt8">
                  <ul className="pl12 pr12 ls1">
                   ...
                   <li className="lead-profile-info df jcsb aic p12">
                      <span className="info-label fs14 fc16">Mobile:</span>

                      {userDetail.mobile_number ? (
                        <span className="v-center fc14 detail-mob">

                          {leadDetail?.initcall?.includes(Number(user?.userid)) ? (
                            <>
                              ...
                              {/* Call Now Button */}
                              <span
                                className={`v-center fs12 mr8 ${!callNow ? "cp fc19" : "fc14"}`}
                                onClick={() => {
                                  if (!callNow) {
                                    handleCallNow("callnow", userDetail);
                                  }
                                }}
                              >
                                {callNow ? (
                          ...
                          >
                            <MdOutlineVerified
                              className={`${
                                userDetail.mobile_status === "1" ? "fc13" : "fc17"
                              } fs18 ml4`}
                            />
                          </Tooltip>

                         
                          {userDetail.whatsapp_status > 0 && (
                            <DynamicTooltip
                              direction="left"
                              text={
                            ...
                            >
                              <MdOutlineWhatsapp
                                className={`${
                                  userDetail.whatsapp_status === "1" ? "fc13" : "fc17"
                                } fs18 ml4`}
                              />
                            </DynamicTooltip>
                          )}
                  ...
                  {otherDetail.paymentcount &&
                      otherDetail.paymentcount > 0 && user.role === '1' && (
                        <li
                      ...
                      {otherDetail.pending_approval_amount!=null && otherDetail.pending_approval_amount>0 && (
                      <li className="lead-profile-info df jcsb aic p12 df jcsb aic p12">
                    ...
                    {leadDetail.last_enquiry_posted	 && (
                      <li className="lead-profile-info df jcsb aic p12 df jcsb aic p12">
                        <span className="info-label fs14 fc16">
                          Last Inquiry Date:
                        </span>
                  ...
                  <div className="payment-buttons df aic jcc fww fs12 ls1 p12 mt16">
                    {/* <div
                        className="resete-pass fc1 pt8 pb8 pl8 pr8 tdu cp mr8 fs14"
                        onClick={handleSendMailPassowrdyn pff
                    ...
                    {otherDetail.invoicestatus && (
                      <div
                        className="pymnt-hist bg5 pt8 pb8 pl8 pr8 fc1 brd2 cp mr8 mb8"
                        onClick={openSendInvoicePopup}
                      >
                        Send invoice
                      </div>
                    )}
                    {(RegistrationButtonPermission || user.role === "1") && (
                      <>
                        {enrolledCoursesStatus && (
                          <div
                            className="pymnt-hist bg5 pt8 pb8 pl8 pr8 fc1 brd2 cp mr8 mb8"
                            onClick={checkingDownloadRegForm}
                          > Download Reg. Form
                          </div>
                        )}
                        {certificateStatus && (
                          <div
                            className="pymnt-hist bg5 pt8 pb8 pl8 pr8 fc1 brd2 cp mr8 mb8"
                            onClick={checkingDownloadCertificate}
        ...
        {updateStausPopup && (
          <Popup onClose={closeUpdateStatusPopup} title={"Update Status"}>
            <UpdateStatus
              ...
              errorMsg={errorMsg}
              setUpdateStausPopup={setUpdateStausPopup} 
              handleClickNavigte={handleClickNavigte}
            />
          </Popup>
        )}
        ...
        user={user}
        userDetail={userDetail}
        checkingDownloadRegFormSts={checkingDownloadRegFormSts}
        setCheckingDownloadRegFormSts={setCheckingDownloadRegFormSts}
        />
        
         <StudentDownloadCertificate
         id={id}
         constant={constant}
         user={user}
         userDetail={userDetail}
         checkingDownloadCertificateSts = {checkingDownloadCertificateSts}
         setCheckingDownloadCertificateSts = {setCheckingDownloadCertificateSts}
         />
