...
const ManageAccount = () => {
  ...
  useTitle("Manage Profile - Flapone Aviation");
  const { id } = useParams();
  ...
  const [fetchUserDept, setFetchUserDept] = useState("");
 ...
 function validateContent(html) {
  ...
  function getSettingData() {
    ...
    axios({
      method: "post",
      url: `${constant.base_url}/admin/manage_account.php?fun=settingData`,
      ...
      .then(function (response) {
        ...
        if (response.data.data.status === "1") {
          ...
          setFetchUserRole(settingData.role_id);
          setFetchUserDept(settingData.fk_dept_id);
          ...
          if (settingData.designation) {
            setDesignation(settingData.designation);
          }
          ...
          if (settingData.gender) {
            setGender(
              genderOptions.find(
                (option) => option.value === settingData.gender
              )
            );
          }
  ...
  const handleFormSetting = (key, value) => {
    axios({
      method: "post",
      url: `${constant.base_url}/admin/manage_account.php?fun=settingUpdate`,
      headers: { "Auth-Id": user.auth_id },
      data: {
        userid: id,
        key: key,
  ...
  const setPincodeFun = (value) => {
    ...
    if (!isValid) {
      setErrorMsgPincode("Pincode must be digit & a 6-digit number.");
      return false;
    }
  ...
  const setCityFun = (value) => {
    ...
    if (!isValid) {
      setErrorMsgCity("Only a-z,A-Z . - & Allowed");
      return false;
    }
  ...
  const setStateFun = (value) => {
    ...
    if (!isValid) {
      setErrorMsgState("Only a-z,A-Z . - & Allowed");
      return false;
    }
  ...
  const setDesignationFun = (value) => {
    value = value.trim();
    handleFormSetting("designation", value);
  ...
  const handleUploadProfileImg = (file) => {
    ...
    formData.set(
      "oldprofileImg",
      profileImg.user_image ? profileImg.user_image : ""
    );
    ...
    axios
      .post(
        `${constant.base_url}/admin/manage_account.php?fun=uploadImg&type=profile`,
        formData,
        {
          headers: {
  ...
  return (
    <div className="account-details  pl20 df w66 fdc settings">
          ...
          <>
            <h3 className="fc1 fw5 fs20 mt24 mb24">
              ...
              <div className="fs14 fc16 mt8 fw4">
                Update & ensure the profile information is accurate and
                up-to-date to maintain account integrity
              </div>
            </h3>
            ...
            <div className="form-group-settings mobile">
              <div className="df jcsb aic">
                ...
                {countryCode === "91" && phoneVerify === "0" ? (
                  <p className="loader-container df aic"></p>
                ) : (
                  <p
                ...
                <div className="input-group-prepend df">
                  <div className="role-dropdown" ref={dropdownRef}>
                    <div
                      ...
                      disabled={phoneVerify === "1" ? true : false}
                    >
              ...
              <>
                <div className="df jce aic">
                  <label htmlFor="addMore" className="v-center">
                    ...
                    />
                    {isChecked === true ? "Unmark" : "Mark"} Profile as
                    Instructor
                  </label>
                </div>
                ...
                {isChecked && (
                  <>
                    <div className="form-group-settings city-pin df jcsb">
                      <div className="Designation flx1">
                        <p className="fc15 fw6 fs14 ls1">Designation</p>
                        <input
                          type="text"
                          id="designation"
                          placeholder="Enter Designation"
                          defaultValue={designation}
                          onBlur={(e) => setDesignationFun(e.target.value)}
                          autoComplete="off"
                        />
                      </div>
                    </div>
            ...
            {((dataStatus && fetchUserDept === "9") || isChecked === true) && (
              <div className="tags  pb48 settings-course" style={{ marginBottom: "200px" }}>
                <p className="fc15 fw6 fs14 ls1 mb12">Select Courses</p>
                <MultiselectDropdown
                  options={instructor}
