...
const HolidayForm = () => {
  ...
  const customDateFormat = "EE, MMM d, yyyy";
  ...
  const validateForm = () => {
    ...
    if (!holidayFormData.holidayDate) {
      ...
      const selectedDate = new Date(moment(holidayFormData.holidayDate, "DD/MM/YYYY").toDate());
  
      if (selectedDate < today) {
        errorList.holiday_date = "Holiday date cannot be earlier than today.";
        isValid = false;
      }
    }
  ...
  const openHolidayList = () => {
    navigate("/my-students?tab=holiday");
  };
  ...
  return (
    <>
    {(filterStatus || pageLoadStatus) && ((pageDeptAccess == "1" || pageDeptAccess == "3" || pageDeptAccess == "5") || pageRoleAccess == "1") ? (
        <>
      <InnerHeader
        heading="Add Holiday"
        txtSubHeading="Use this form to add a holiday."
        showButton={true}
