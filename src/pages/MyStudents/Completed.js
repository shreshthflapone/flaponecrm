...
import { IoIosAirplane, IoIosBook, IoIosPeople, IoLogoCodepen, IoLogoXbox, IoMdListBox, IoMdPaper } from "react-icons/io";
...
import ImagePdfUpload from "../../components/ImagePdfUpload.js";
...
const Completed = ({
  ...
  useTitle("Course Completed - Flapone Aviation");
...
const convertDateToReactFormat = (batchStartDate, batchEndDate) => {
    ...
    if (Object.keys(updates).length > 0) {
      ...
      if (assignedBatch.value > 0) {
        ...
        allStudentsData.map((lead) => {
          if (selectedLeads.includes(lead.id)) {
            allstudentid.push(lead.id+"|"+lead.wo_id);
          }
        });
  ...
  function handleOpenLink(student, type){
    let generateLink = constant.doc_url+student.allotted_batch + "/" + student.id + "/" + student.courseId + "/" + student[type];
        ...
        <table className="mylead-table cp wsnw">
          <thead className="w100">
            <tr>
              ...
              <th onClick={() => handleSortByChange("dgca_batch_id")} className={activeSortColumn === "dgca_batch_id" ? "fc1" : ""}>
                <p className="box-center">
                  DGCA <br/>Batch
                  <RiArrowUpDownFill className="cp ml4" />
                </p>
              ...
              allStudentsData.map((student, index) => {
                return (
                  <tr key={index} onClick={() => handleRowClick(student.id)}>
                    ...
                    {student.coordinatorid ? coordinatorOptions.find(agent => agent.value === student.coordinatorid)?.label || "-" : "-"}
                      {/* <Tooltip
                        title={
                    ...
                    <td className="leads-tool-fix">{student.dgca_batch_id? student.dgca_batch_id:"-"}</td>
                    <td className="leads-tool-fix">{student.rpc_number ? <p onClick={(e)=>{
                      e.stopPropagation();
                      handleOpenLink(student,'rpcCertificate');
                    }}> <Tooltip
                        title="Click To Open Certificate"
                      >{student.rpc_number?student.rpc_number:"-"}</Tooltip></p>:"-"}</td>
                   
                   <td className="leads-tool-fix">{student.coc_roll_no ? <p onClick={(e)=>{
                            e.stopPropagation();
                            handleOpenLink(student,'coc')
                    }}> <Tooltip
                        title="Click To Open COC"
                      >{student.coc_roll_no}</Tooltip></p>:"-"}</td>
  ...
  ? batchOptions[student.courseId]?.find((batchopt) => batchopt.value === student.allotted_batch)?.label || "-":"-"}
                      ...
                      >
                        <select
                          ...
                          onChange={(event) => {
                            handleAssignToDropdownChange(event, student.id+"|"+student.wo_id,'allotted_batch');
                          }}
                          onClick={(e) => {
                            e.stopPropagation();
                          }}
                    ...
                    <td className="leads-tool-fix">{student.issue_date}</td>
                    <td>
                    <p className="v-center">
                      {student.flyingLog ? <Tooltip
                        title="Flying Log"
                      ><IoIosAirplane
                        className="icon cp fs20 fc5 mr8"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenLink(student,'flyingLog');
                        }}
                      /></Tooltip>:"-"}
                       {student.attedanceLog ? <Tooltip
                        title="Attedance Log"
                      ><IoIosPeople
                        className="icon cp fs20 fc5 mr8"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenLink(student,'attedanceLog');
                        }}
                      /></Tooltip>:"-"}
                       {student.simLog ? <Tooltip
                        title="Simulator Log"
                      ><IoLogoXbox
                        className="icon cp fs20 fc5 "
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenLink(student,'simLog');
                        }}
                      /></Tooltip>:"-"} </p></td>
                    <td>
                    <p className="v-center">
                      {student.rpcTheoryTest ? <Tooltip
                        title="RPCTheory Test"
                      ><IoLogoCodepen
                        className="icon cp fs20 fc5 mr8"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenLink(student,'rpcTheoryTest');
                        }}
                      /></Tooltip>:"-"}
                       {student.skillTest ? <Tooltip
                        title="Skill Test"
                      ><IoMdPaper
                        className="icon cp fs20 fc5 mr8"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenLink(student,'skillTest');
                        }}
                      /></Tooltip>:"-"}
                       {student.progressTest ? <Tooltip
                        title="Progress Test"
                      ><IoIosBook
                        className="icon cp fs20 fc5 "
                        onClick={(e) => {
                          e.stopPropagation();
                          handleOpenLink(student,'progressTest');
                        }}
                      /></Tooltip>:"-"}</p></td>
                     ...
                     <td>
                      ...
                      <HiOutlineDocumentCheck
                        className={` cp fs20 mr8 p6 ${student.doc_status === "1" ? "fc2" : "fc5"}`}
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      >
                       <IoMdListBox
                        className="icon cp fs20 p6 fc5 mr8"
                        onClick={(e) => {
                          e.stopPropagation();
         ...
         {instructorFormShow && !isLoading && (
          ...
          <p className="error-text tac">{courseDocStatusMessage &&(
                    ...
                    {holdMarkRecord.course_category_name==='drone' && <div className="pl32 w100 inst-upload-form">
                      <div className="df fww inst-form-upload jcsb mt16 course-com-mark">
                        <div className="flx22 form-group-settings mr32 w100">
                          ...
                          <div className="class-calendar">
                            <DatePicker
                              ...
                              selected={instructorSubmitForm.issueDate ? instructorSubmitForm.issueDate : parse(holdMarkRecord.isse_date, "d LLL yyyy", new Date())}
