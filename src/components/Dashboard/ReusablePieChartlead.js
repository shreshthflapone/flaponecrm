...
const ReusablePieChartlead = ({
  ...
  const handleClick = async (entry) => {
    ...
    if (title !== 'Follow-up' && title !== 'WhatsApp Leads' && entry.level < 3) {
      await getTraverserNextlevele(entry, nextBreadcrumb);
    }
  ...
  const getStatusChecked = (filter_name)=>{
    if(filter_name=='Follow-up' || filter_name=="followupdate"){
  ...
  function parseDateRangeString(dateRangeStr) {
    ...
    const startDate = new Date(startDateObj.setHours(18, 30, 0, 0)).toISOString();
  
   
    const endDate = new Date(endDateObj.setHours(18, 29, 59, 999)).toISOString();
  ...
  const handleRedirectSetFilter = (pageTab, dateOption, dateRangeValue,item) => {
    let updatefilter = {
      ...
      dateOption: item?.date_type=="wo_create_date"?dateOption:"",
      ...
      selectedState:item?.selectstate?item?.selectstate:[],
      categoryCheckedItems: item?.categorycheck?item?.categorycheck:listFilter.categoryCheckedItems,
      userType:item?.user_type?item?.user_type:[],
      selectedCourses: [],
      companyType: {
        label: "User Type",
        value: "",
      },
      workOrderStatus: (item?.source=='whatsapp' || item?.tab=='workorder')?["pending", "success"]:[],
      leadVerified:  item?.source=='whatsapp'?{label: "Responded via WhatsApp", value: "whatapps_verified"}:{label: "Select Verified", value: ""},
      selectedVerified: item?.source=='whatsapp'?{label: "Responded via WhatsApp", value: "whatapps_verified"}:{label: "Select Verified", value: ""},
      dateRangeValuefilter: parseDateRangeString(item.source=="followupdate"?item.set_key:`${format(dateRangeValue[0].startDate, "dd-MM-yyyy")} | ${format(dateRangeValue[0].endDate, "dd-MM-yyyy")}`),
      dateMonthOptions: {
        label: "Day",
        value: "day",
      },
      dateRangeTimeType: "Custom",
      statusCheckedItems : item?.filter_type=='card'?item.filter_card:getStatusChecked(item.source=="followupdate"?item.source:item.set_key),
    };
