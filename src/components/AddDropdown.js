...
const AddDropdown = ({
  ...
  noLabel = "true",
  placeholder = "Select",
  allowCustom = false, // New prop for allowing custom input
}) => {
      ...
      <div className="role-dropdown" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="dropdown-content searching-drop">
            ...
            {allowCustom && searchTerm && !filteredOptions.some(option => option.label === searchTerm) && (
              <div className="add-custom-option p10 tac cp brd-b1 bg1 fc3" onClick={handleAddCustomOption}>
                Add "{searchTerm}"
              </div>
            )}
          </div>
